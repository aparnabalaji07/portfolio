  ...
  return (
    <dl className="mt-10 border-t border-white/10 font-mono text-sm max-w-2xl">
      {detail.mechanics.map((m) => (
        <div
          key={m.term}
          className="grid grid-cols-1 md:grid-cols-[9rem_1fr] gap-1 md:gap-6 border-b border-white/10 py-4"
        >
...
function HardwareBody({ detail }) {
  return (
    <>
      <div className="mt-10 grid grid-cols-2 border-t border-l border-white/10 max-w-2xl">
        {detail.specs.map((s) => (
          <div key={s.label} className="border-r border-b border-white/10 p-5">
